...
export async function PATCH(
  ...
  try {
    ...
    if (typeof body.is_treated !== 'boolean') {
      return NextResponse.json(
        { error: 'Le champ is_treated doit être un boolean (true pour traiter, false pour détraiter)' },
        { status: 400 }
      );
    }
    ...
    if (typeof resumeService.updateResumeTreatmentStatus !== 'function') {
      console.error('updateResumeTreatmentStatus is not available:', Object.keys(resumeService));
      return NextResponse.json(
    ...
    const success = await resumeService.updateResumeTreatmentStatus(id, body.is_treated);
    ...
    return NextResponse.json(
      {
        success: true,
        message: body.is_treated ? 'CV marqué comme traité' : 'CV marqué comme non traité',
