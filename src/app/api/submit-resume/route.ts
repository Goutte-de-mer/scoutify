...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.resume || !body.player_profile || !body.contacts) {
      return NextResponse.json(
        { error: 'Données manquantes: resume, player_profile et contacts sont requis' },
        { status: 400 }
      );
    }
    ...
    if (!body.player_profile.first_name || !body.player_profile.last_name || 
        !body.player_profile.birth_date || !body.player_profile.photo_path ||
        body.player_profile.weight_kg === undefined || body.player_profile.weight_kg === null ||
        !body.player_profile.main_position) {
      return NextResponse.json(
        { error: 'Le profil joueur doit contenir: first_name, last_name, birth_date, photo_path, weight_kg et main_position' },
        { status: 400 }
      );
    }
    
    // Validation des champs numériques du profil
    if (body.player_profile.height_cm === undefined || body.player_profile.height_cm === null) {
      return NextResponse.json(
        { error: 'Le profil joueur doit contenir height_cm' },
        { status: 400 }
      );
    }
    ...
    if (!body.player_profile.strong_foot) {
      return NextResponse.json(
        { error: 'Le profil joueur doit contenir strong_foot' },
        { status: 400 }
      );
    }
    ...
    if (!body.contacts.player_email || !body.contacts.player_phone) {
      return NextResponse.json(
        { error: 'Les contacts doivent contenir au minimum: player_email et player_phone' },
        { status: 400 }
      );
    }
    ...
    if (body.contacts.agent_email && !emailRegex.test(body.contacts.agent_email)) {
      return NextResponse.json(
    ...
    if (body.qualities && !Array.isArray(body.qualities)) {
      return NextResponse.json(
        { error: 'qualities doit être un tableau' },
        { status: 400 }
      );
    }
    ...
    if (body.career_seasons && !Array.isArray(body.career_seasons)) {
      return NextResponse.json(
        { error: 'career_seasons doit être un tableau' },
        { status: 400 }
      );
    }
    ...
    if (body.training_entries && !Array.isArray(body.training_entries)) {
      return NextResponse.json(
        { error: 'training_entries doit être un tableau' },
        { status: 400 }
      );
    }
    ...
    if (body.club_interests && !Array.isArray(body.club_interests)) {
      return NextResponse.json(
        { error: 'club_interests doit être un tableau' },
        { status: 400 }
      );
    }
